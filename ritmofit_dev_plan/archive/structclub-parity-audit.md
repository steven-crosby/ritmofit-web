...
# Ritmo Studio Web — StructClub Feature-Parity Audit
...
treated as real gaps **but flagged** where a recommendation collides with a locked Ritmo Studio constraint.

---

## 0. Read this first

**The product DNA already matches.** Ritmo Studio is explicitly built as a StructClub competitor for rhythm
...
live playback. Ritmo Studio deliberately splits these — `ritmofit-web` is the **planning** surface, the native
...
| # | StructClub surface | Ritmo Studio web | Status | One-line gap |
|---|---|---|---|---|
...
## 2. Where Ritmo Studio already EXCEEDS StructClub
...
- **Ritmo Studio:** `ExploreDialog` lists public classes grouped by `featuredCategory` string; each row is text-only (title · author · track count) with **Preview** (read-only track list) + **Save a copy**.
...
- **Ritmo Studio:** `LibraryRail` rows are `title` + `accessLevel` only; create is a single "New class title" + Add field. Share/Delete live in the header of an *opened* class; "Save a copy" (duplicate) exists only from Explore/Preview.
...
- **Ritmo Studio:** owners get the full **3-pane editor**; the only read-only view is `ClassSummaryView` (Explore preview), which lists tracks but **not** the interleaved moves/cues/sections.
...
- Ritmo Studio's `TrackInspector` + `ChoreographyEditor` cover Move (library + custom + one-off), Intensity, Display-BPM (+ "Look up BPM"), Duration, Notes, and anchored Cues with color tags — and **add** trim, downbeat/beat-snap, and bar.beat. Verified live: added a cue at 0:30 and a Climb/hard move at 1:00.
...
- **Ritmo Studio:** moves can be **placed** on tracks and **managed** (`CustomMovesDialog`), but there is **no reverse index** — no endpoint or UI to ask "which songs/classes use this move?" Confirmed: no such route in `apps/api/src/routes/` and no UI affordance.
...
- **Ritmo Studio:** `LiveMode` is a **cue prompter** — virtual-clock Play/Pause/Reset + seek, NOW/NEXT cards, intensity bars, countdowns, a Full-List move view, and an **"Open in SoundCloud" deep-link** instead of embedded playback. Verified live.
- **The collision:** an embedded provider transport / now-playing bar requires controlling provider playback in-app, which **directly violates Ritmo Studio's locked constraint**: *"No in-app audio mixing/crossfade… audio plays through the user's own provider apps; we deep-link/hand off… never stream or mix."* (`DEVELOPMENT_PLAN.md` → Hard constraints).
...
- **Ritmo Studio:** `TeamsDialog` creates teams + manages members and serves as a **share target**, but there's no "browse my team's classes" feed. Plus it's **404 on prod** right now.
- **Recommend:** P0 fix the 404; P2 consider a team-classes browse view if team discovery matters.

### H. Settings — 🟡 minor
- StructClub has a gear (top-left) on Explore/Library. Ritmo Studio has only "Sign out." No profile, theme, or account settings surface. Low priority.
...
- Ritmo Studio seeds **generic gym/tread** moves (Bicep Curl, Burpee, Climb, Incline Walk, Jog, Jumps, Mountain Climber, Push, Recovery, Run, Sprint, Sprint Hold, Squat, Tap Back). StructClub's are **rhythm-cycle** (Left Leg Lead, Right Leg Lead, Oos, Walk Your Hands, Sway, Isolate, Hands In & Out). Since the dev plan positions Ritmo Studio for "rhythm spin cycle instructors," the seed library is arguably off-target. Custom moves bridge it, but the default vocabulary is a quick, high-leverage content fix (re-seed `moves`).

---

## 4. Things Ritmo Studio has that StructClub's screenshots don't show
Multi-provider import, email/team sharing with permissions, beat-snapping, trim, free placement, drag/keyboard
a11y, tags, cue color tags, wake-lock. Net: **Ritmo Studio's builder is ahead; its discovery/library/onboarding is behind.**
