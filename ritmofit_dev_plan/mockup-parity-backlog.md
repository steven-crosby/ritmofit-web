...
| # | Item | Gap vs mockup | Effort | Evidence |
|---|---|---|---|---|
| M1 | **Intensity vocabulary + segmented control** | ✅ **Decided (D17): zone vocab** — Z0 None / Z1 Build / Z2 Push / Z3 Attack / Z4 All Out. Display-only relabel of `INTENSITY_LABEL` (no migration) **+** flip the inspector/manual-add control from raw-enum dropdown → labeled segmented control. | S–M | `IntensityReadout.tsx`; `builder.html` |
...
|---|---|---|---|---|
| P1 | **Empty-state-as-error (systemic)** | Explore, Teams, **and** Share all render the empty state as **"Not found." in danger-red** + a lingering **"Loading…"**. One shared fix. | S | live: Explore/Teams/Share dialogs |
| P2 | **Raw enum in intensity `<select>`** | Dropdown shows `mod`, `all_out` instead of human labels (manual-add form + inspector). Folds into M1's segmented-control swap. | XS | `Dashboard.tsx` add-track + inspector selects |
