...
- **Web:** CSS custom properties (`--rf-*`) consumed by React components.
- **iOS:** a Swift constants file (colors, radii, spacing, type, motion, tempo) consumed by SwiftUI/UIKit.

A build step transforms the JSON into each target. **Never hand-edit values in component code** — change
the token and regenerate. This keeps the two clients in lockstep as the brand evolves.
...
- Motion durations/easing and the signature microinteractions' *intent*.
...
| Icons | Material Symbols (rounded) | SF Symbols (rounded equivalents) |
...
- Don't force web chrome onto iOS or iOS gestures onto web. Same *language*, native *grammar*.
